...
struct AnimateVideoLayer {
    ...
    func add(_ newLayer:CALayer, to layer: CALayer, start:CGFloat, duration:CGFloat, properties:AnimationProperties) {
        newLayer.opacity = 0
        appeareAnimation(.opacity, duration: duration, newLayer: newLayer, start: start)
    ...
    private func appeareAnimation(_ type:AppeareAnimationType, duration:CGFloat, newLayer:CALayer, start:CGFloat) {

        let show = basicAppeare(type, show: true, start: start)//duration * 0.2
        newLayer.add(show, forKey: "show")
        let hide = basicAppeare(type, show: false, start: duration + start)//duration * 0.8
