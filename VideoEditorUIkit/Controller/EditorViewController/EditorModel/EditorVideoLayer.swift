...
struct EditorVideoLayer {
    ...
    func addLayer(to layer: CALayer, videoSize: CGSize, text:TextAttachmentDB) {
        let newValue = attachmentLayer.add(to: layer, videoSize: videoSize, text: text)
        animation.add(newValue, to: layer, start: text.inMovieStart, duration: text.duration, properties: .init(needScale: text.needScale))
    }
    ...
    func addLayer(video: String, to layer: CALayer, videoSize: CGSize, videoDuration:CGFloat) {
        ...
        animation.add(newValue, to: layer, start: videoDuration * 0.2, duration: videoDuration, properties: .init(needScale: false))
    }
}
