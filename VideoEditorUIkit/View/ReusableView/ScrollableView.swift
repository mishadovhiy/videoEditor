...
struct ScrollableView<Content: View>: UIViewControllerRepresentable, Equatable {
    ...
    // MARK: - Init
    init(_ offset: Binding<CGPoint>, animationDuration: TimeInterval = 0.3, showsScrollIndicator: Bool = true, axis: Axis = .vertical, onScale: ((CGFloat)->Void)? = nil, disableScroll: Bool = false, forceRefresh: Bool = false, stopScrolling: Binding<Bool> = .constant(false),  @ViewBuilder content: @escaping () -> Content) {
        self.offset               = offset
    ...
    func updateUIViewController(_ viewController: UIViewControllerType, context: UIViewControllerRepresentableContext<Self>) {
        ...
        print("newValuenewValueofst ", newValue)
